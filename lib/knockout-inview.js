...
const addListenerObservable = function (element, observable, offset) {

  const listener = function () {

    const rect = element.getBoundingClientRect();
    const top = offset === "bottom-in-view" ? rect.bottom : rect.top;
    const isInview = isOverlapping(top, rect.bottom, 0, window.outerHeight);
...
const addListenerCallback = function (element, callback, offset) {
...
const binding = function (ko) {
  ...
  const init = function (element, valueAccessor) {
    ...
    if (ko.isObservable(handler)) {
      addListenerObservable(element, handler, offset);
    } else if (handler instanceof Function) {
      addListenerCallback(element, handler, offset);
    }
